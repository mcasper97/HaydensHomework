...
import { normalizeGoogleCalendarRouting, buildGoogleCalendarRoutingSave, saveGoogleCalendarRouting } from "./data/googleCalendarRouting.js";
 ...
 * Connect/disconnect/status only — mirrors GmailConnectionPanel's exact
 * structure, deliberately WITHOUT an emailAddress display (Phase 1 never
 * requests an identity scope or stores/shows the connected account's
 * email for Calendar; "Google Calendar connected" is enough, per the
 * Slice B design). No publish/event UI of any kind belongs here yet — see
 * the module doc comment in src/data/googleCalendarConnection.js.
 */
 ...
 * Replaces Slice C.1A's temporary "Test Calendar List" live-validation
 * control (now removed) with the real routing-configuration UI, built on
 * the exact same authenticated getCalendarList() path that control
 * validated live. Lets a parent choose which Google calendar each
 * learner (and the family/default) publishes to — see
 * src/data/googleCalendarRouting.js for the persisted shape and
 * src/organizer/calendarRouting.js for how a publish action will resolve
 * it (Slice C.1C, not this slice — this panel only saves configuration,
 * it never touches calendar-publish.js or routes any Item).
 *
 * Only rendered when Calendar is connected at all (a connection record
 * exists) — see its render site below. Within that, three distinct
 * states the fetched Calendar List can be in:
...
const ChildSelector = ({
  ...
  // Google Calendar routing config (Slice C.1B) — loaded alongside the
  // rest of the household profile below (see finish()), normalized via
  ...
  // Guest/admin mode never connects Google Calendar at all (see
  // GoogleCalendarConnectionPanel/GoogleCalendarRoutingPanel's own
  // real-account-only gating), so this is never read from localStorage —
  // it simply stays at its default there, matching how those panels
  // never render in guest mode either.
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6" style={{ background: "#1C1C1E" }}>
      <div className="w-full max-w-lg">
        ...
        {/* Google Calendar connect/disconnect/status only (Slice B) — no
            publishing, no event API calls, no Item Calendar fields yet.
            Gated identically to Gmail: requires a real Firebase-
            authenticated parent (guest/local mode has no account to
            attach a Calendar connection to), Parent Tools only — never
            reachable from Family Board, Child Home, or a locked/shared
            Display surface. */}
        {showParentTools && !user.isAdmin && <GoogleCalendarConnectionPanel />}

        {/* Google Calendar routing configuration (Slice C.1B) — which
            calendar each learner/family item publishes to. Gated
            identically to GoogleCalendarConnectionPanel just above (real
            Firebase-authenticated parent, Parent Tools only, never
            reachable from Family Board/Child Home/a locked Display
            surface); the panel itself additionally renders nothing at all
            unless Calendar is actually connected (see its own doc
            comment). Supersedes Slice C.1A's temporary "Test Calendar
            List" validation control, now removed. */}
        {showParentTools && !user.isAdmin && (
