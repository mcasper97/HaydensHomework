...
- timeMode / daypart / time (only meaningful when recurring is true): the source almost never gives an exact clock time for a recurring instruction — most say only "morning", "before school", "nightly", or "evening". Capture that as a semantic daypart only: timeMode: "daypart" with daypart: "morning" or daypart: "evening" (map "before school"/"morning" to "morning"; "nightly"/"evening"/"after school" to "evening"). Only use timeMode: "exact" with a time value if the source states an actual clock time for the recurring instruction itself (rare) — never invent one. If the source gives no time signal of any kind for a recurring obligation, leave timeMode, daypart, and time all null — do not default to a daypart or a time just because the obligation is recurring.
